...
(async () => {
    var msg = await redisCli.pipeline().lindex('room_4j2e8xk1uimg1gir_recentmsg', 2).exec();
    console.log(msg);
    ...
    if(d.from.name.indexOf('游客') > -1){
        ...
        d.from.uid = 20;
        var res = await redisCli.pipeline().lset('room_4j2e8xk1uimg1gir_recentmsg', 2, JSON.stringify(d)).exec();
        console.log(res);
    }
